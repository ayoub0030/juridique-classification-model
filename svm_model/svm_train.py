...
"""
SVM Model Training Script for Legal Document Classification
This script trains a Support Vector Machine model on legal documents for classification.
"""
...
def train_svm_model(data_path, output_dir, test_size=0.2, max_features=10000, 
                   ngram_range=(1, 2), C=1.0, class_weight='balanced', 
                   random_state=42, perform_grid_search=False):
    """
    ...
    print("Creating TF-IDF vectorizer and SVM pipeline...")
    ...
    text_clf = Pipeline([
        ...
        ('clf', LinearSVC(C=C, class_weight=class_weight, random_state=random_state, max_iter=10000))
    ])
    ...
    for i, class_name in enumerate(classes):
        ...
        class_coefficients = coefficients[i] if len(classes) > 2 else coefficients[0]
...
def main():
    parser = argparse.ArgumentParser(description='Train an SVM model for text classification')
